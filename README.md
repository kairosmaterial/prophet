...
Checkpoints are distributed on the
[Hugging Face model repository](https://huggingface.co/kairosmaterial/prophet)
(custom format: one-line JSON config header followed by the PyTorch state dict)
and load via `prophet.load_model(path)` or directly through the calculators:
